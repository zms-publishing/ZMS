  ...
  def test_get(self):
    ...
    for document in self.context.getTreeNodes(MockHTTPRequest(), 'ZMSDocument'):
        ...
        for path_to_handle in [list(document.getPhysicalPath()), [name, document.get_uid()]]:
            # multilingual
            self.context.REQUEST = MockHTTPRequest({'REQUEST_METHOD':'GET','TraversalRequestNameStack':path_to_handle,'path_to_handle':path_to_handle})
            ...
            # list_parent_nodes
            self.context.REQUEST = MockHTTPRequest({'REQUEST_METHOD':'GET','TraversalRequestNameStack':path_to_handle+['list_parent_nodes'],'path_to_handle':path_to_handle+['list_parent_nodes']})
            print("path_to_handle", self.context.REQUEST.get('path_to_handle'))
            actual = json.loads( self.context.__bobo_traverse__(self.context.REQUEST, name)(self.context.REQUEST))
            print(json.dumps(actual))
            self.assertEqual( len(actual), len(document.breadcrumbs_obj_path()))
            # list_child_nodes
            self.context.REQUEST = MockHTTPRequest({'REQUEST_METHOD':'GET','TraversalRequestNameStack':path_to_handle+['list_child_nodes'],'path_to_handle':path_to_handle+['list_child_nodes']})
            print("path_to_handle", self.context.REQUEST.get('path_to_handle'))
            actual = json.loads( self.context.__bobo_traverse__(self.context.REQUEST, name)(self.context.REQUEST))
            print(json.dumps(actual))
            self.assertEqual( len(actual), len(document.getChildNodes(self.context.REQUEST)))
            # list_tree_nodes
            self.context.REQUEST = MockHTTPRequest({'REQUEST_METHOD':'GET','TraversalRequestNameStack':path_to_handle+['list_tree_nodes'],'path_to_handle':path_to_handle+['list_tree_nodes']})
            print("path_to_handle", self.context.REQUEST.get('path_to_handle'))
            actual = json.loads( self.context.__bobo_traverse__(self.context.REQUEST, name)(self.context.REQUEST))
            print(json.dumps(actual))
            self.assertEqual( len(actual), len(document.getTreeNodes(self.context.REQUEST)))
            # get_child_nodes + multilingual
            self.context.REQUEST = MockHTTPRequest({'REQUEST_METHOD':'GET','TraversalRequestNameStack':path_to_handle+['get_child_nodes'],'path_to_handle':path_to_handle+['get_child_nodes']})
            print("path_to_handle", self.context.REQUEST.get('path_to_handle'))
            actual = json.loads( self.context.__bobo_traverse__(self.context.REQUEST, name)(self.context.REQUEST))
            print(json.dumps(actual))
            self.assertTrue( isinstance( actual, list))
            ...
            # get_tree_nodes
            self.context.REQUEST = MockHTTPRequest({'REQUEST_METHOD':'GET','TraversalRequestNameStack':path_to_handle+['get_tree_nodes'],'path_to_handle':path_to_handle+['get_tree_nodes']})
            print("path_to_handle", self.context.REQUEST.get('path_to_handle'))
            actual = json.loads( self.context.__bobo_traverse__(self.context.REQUEST, name)(self.context.REQUEST))
            print(json.dumps(actual))
            # TODO implement here
