...
This module implements asynchronous content reindexing for ZMS by traversing ZMSIndex candidates, retrieving schematized content through the REST API, and forwarding documents to the configured search connector (for example OpenSearch).
...
5. `ZMSIndexSchematizedReindexer.run()`:
	 ...
	 - calls REST endpoint `get_indexschematized_content` per UID and language
	 - resolves returned docs back to ZMS nodes
	 - sends `(node, data)` tuples to catalog connector
...
- `connector.manage_objects_add(objects)`
	- submits normalized documents for indexing
