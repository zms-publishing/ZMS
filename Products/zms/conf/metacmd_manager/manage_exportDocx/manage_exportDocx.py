  ...
  for i in r:
    if len(l) == 0:
      ...
      if l[-1]['pr'] == i['pr']:
        ...
        l.append(i)
  return [x for x in l if len(x['content'].strip())>0]
...
def process_regions(html,):
  ...
  r = find_regions(dom,lpr=list(dpr))
  r = join_regions(r,lpr=list(dpr))
  docbookxml = ''
  for i in r:
    d = {}
    pr = i['pr']
    ...
    if len(content.strip()) > 0:
      ...
      for ti in t:
        tn = ti['name']
        ta = ti.get('attrs',{})
        docbookxml += '<%s %s>\n'%(tn,' '.join(['%s="%s"'%(x,ta[x]) for x in ta]))
      # <w:rPr>
      lpr = []
      for ipr in pr:
...
+'\n\t'.join(['<w:num w:numId="%i"><w:abstractNumId w:val="%i"/></w:num>'%(nums.index(x)+1,x['abstract']) for x in nums])
+'</w:numbering>')
  f.close()
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
   ...
   (
    '\n\t'.join(['<Relationship Id="rId%i" %s/>'%(rels.index(rel)+1,' '.join(['']+['%s="%s"'%(x,rel[x]) for x in rel])) for rel in rels])
   )
  )
  f.close()
