				...
				$.plugin('multiselect_js').get("select.zmi-select[multiple]:not(.hidden)",function(){
						$("select.zmi-select[multiple]:not(.hidden)",context).each(function() {
								...
								html += ''
									...
									+ '</div><!-- .col-xs-5 -->'
									+ '<div class="col-xs-2">'
									+ '<button type="button" id="'+multiselect_id+'_rightAll" class="btn btn-secondary btn-block">'+$ZMI.icon('icon-double-angle-right')+'</button>'
									+ '<button type="button" id="'+multiselect_id+'_rightSelected" class="btn btn-secondary btn-block">'+$ZMI.icon('icon-angle-right')+'</i></button>'
									+ '<button type="button" id="'+multiselect_id+'_leftSelected" class="btn btn-secondary btn-block">'+$ZMI.icon('icon-angle-left')+'</i></button>'
									+ '<button type="button" id="'+multiselect_id+'_leftAll" class="btn btn-secondary btn-block">'+$ZMI.icon('icon-double-angle-left')+'</button>'
									+ '</div><!-- .col-xs-2 -->'
									+ '<div class="col-xs-5">'
