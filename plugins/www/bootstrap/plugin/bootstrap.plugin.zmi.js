			...
			$(".accordion-body").each(function() {
					...
					if (typeof id != "undefined") {
						...
						if ($toggle.length > 0) {
							...
							var value = $ZMILocalStorageAPI.get(key,'0');
							if (value != null) {
								if ($(this).hasClass('in')) {
									if (value == '0') {
										$(this).removeClass('in');
		...
		.each(function() {
			...
			// Multiselect
				$.plugin('multiselect',{
					context: context,
					files: [
						//$ZMI.getConfProperty('plugin.bootstrap.multiselect.js','/++resource++zms_/bootstrap/plugin/multiselect.wrapper.js') //+'?ts='+new Date()
