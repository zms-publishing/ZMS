...
def manage_repository_commit(self, request=None):
	...
	printed.append(self.zmi_body_header(self,request,options=[{'action':'#','label':'GIT-%s...'%self.getZMILangStr('BTN_COMMIT')}]))
	...
	if btn=='BTN_COMMIT':
		...
		command = 'git commit -a -m "%s"\ngit push'%(request.get('message'))
	...
	# -------------------------
		...
		printed.append('<button type="submit" name="btn" class="btn btn-primary" value="BTN_COMMIT">%s</button>'%(self.getZMILangStr('BTN_COMMIT')))
