...
class CustomJSONEncoder(json.JSONEncoder):
	def default(self, obj):
		...
		if isinstance(obj, _blobfields.MyBlob):
			REQUEST = obj.REQUEST or {} # Ensure REQUEST is available
			try:
				return {
					'href': obj.getHref(REQUEST),
					'filename': obj.getFilename(),
					'content_type': obj.getContentType(),
					'size': obj.get_size(),
					'icon': standard.getMimeTypeIconSrc(obj.getContentType())
				}
