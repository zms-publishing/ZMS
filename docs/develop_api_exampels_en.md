...
## 7. getObjOptions()
The the default values of the attribute types _select_ or _multiselect_ can be configured in two ways:
...
The second case is practical if the label value of the select list may change sometimes or it contains multiple words (ending up in unreadable template code).
...
_Screen shot: the content model uses a select list for declaring the contents category. The list values are given a tuple: The ZMS-GUI (ZMI) allways show the 2nd value but store the corresonding first value of the tuple. To show the label text by a HTML/TAL template a dict of of all values is needed to gain the lable text by the stored value._
