...
html.append('<input type="hidden" name="form_id" value="manage_addClient"/>')
...
if request.form.get('btn')=='BTN_EXECUTE':
	...
	message.append('</ol>')
	request.response.redirect(context.url_append_params('manage_main',{'lang':request['lang'],'manage_tabs_message':'\n'.join(message)}))
